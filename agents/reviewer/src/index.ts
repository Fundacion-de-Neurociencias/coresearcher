...
export class ReviewerAgent {
   ...
   */
  async validateDOI(doi: string): Promise<{
    valid: boolean;
    confidence: number;
    ...
    try {
      ...
      errors.push(`DOI resolution error: ${(error as Error).message}`);
   ...
   */
  async validatePMID(pmid: string): Promise<{
    valid: boolean;
    confidence: number;
    metadata?: Record<string, unknown>;
    errors?: string[];
    ...
    try {
      ...
      const result = (data?.result as Record<string, unknown>)?.[pmid] as Record<string, unknown> | undefined;
      ...
      const metadata = {
        ...
        authors: (result.authors as Array<{ name: string }>)?.map((a: { name: string }) => a.name),
        journal: result.source as string,
        year: result.pubdate as string,
        uid: pmid,
      };
      ...
      errors.push(`PMID validation error: ${(error as Error).message}`);
   ...
   */
  needsReview(content: string): { needs: boolean; reason: string } {
  ...
  async review(params: {
    content: string;
    citations: string[];
    statisticalResults?: Array<{ type: string; value: string }>;
  }): Promise<{
    reviewed: boolean;
    ...
    const citationResults = await Promise.all(
      params.citations.map(async (citation) => {
  ...
  getMCPRegistration(): MCPAgent {
    const validateDOITool: MCPTool = {
      ...
      handler: async (args: Record<string, unknown>) => {
        return this.validateDOI(args.doi as string);
      },
    };

    const validatePMIDTool: MCPTool = {
      name: 'validate_pmid',
      ...
      handler: async (args: Record<string, unknown>) => {
        return this.validatePMID(args.pmid as string);
      },
    };

    const validateCitationTool: MCPTool = {
      ...
      handler: async (args: Record<string, unknown>) => {
        return this.validateCitation(args.citation as string);
      },
    };
    ...
    const reviewTool: MCPTool = {
      ...
      handler: async (args: Record<string, unknown>) => {
        return this.review({
          ...
          citations: args.citations as string[],
          statisticalResults: args.statisticalResults as Array<{ type: string; value: string }> | undefined,
        });
      },
    };

    const needsReviewTool: MCPTool = {
      name: 'needs_review',
      ...
      handler: async (args: Record<string, unknown>) => {
        return this.needsReview(args.content as string);
      },
    };
