...
export class ProvenanceEngine {
    ...
    linkExecutionToExperiment(executionId, experimentId) {
    ...
    getProvenanceLineage(artifactId) {
    ...
    queryProvenance(query) {
        ...
        if (query.agentId) {
            results = results.filter(r => r.execution && r.execution.agentId === query.agentId);
        }
        if (query.type) {
        ...
        if (query.tags && query.tags.length > 0) {
            results = results.filter(r => query.tags.some(tag => r.artifact.tags.includes(tag)));
        }
    ...
    getExperimentExecutions(experimentId) {
        const experiment = this.experiments.get(experimentId);
        if (!experiment) {
            return [];
        }
    ...
    generateReport(artifactId) {
        ...
        if (artifact.sources && artifact.sources.length > 0) {
            ...
            for (const source of artifact.sources) {
                ...
                if (source.uri)
                    report += `    URI: ${source.uri}\n`;
                if (source.hash)
                    report += `    Hash: ${source.hash}\n`;
            }
            report += '\n';
        }
    ...
    createProvenanceRecord(artifact) {
        const record = {
            id: uuidv4(),
            artifact,
